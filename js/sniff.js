define(["dojo/has"], function (has) { /*App capabilities*/
  ...
  has.add("legend", function (g) {
    ...
    if (g.config.legendlayers) {
      legend = false;
    }
    return legend;
  });
  has.add("legendlayers", function (g) {
    var legendlayers = g.config.legendlayers;
    return legendlayers;
  });
  ...
  has.add("drawer", function (g) {
    var drawer = false;
    if (g.config.legend || g.config.details || g.config.popup_sidepanel || g.config.legendlayers) {
      drawer = true;
    }
    return drawer;
  });
